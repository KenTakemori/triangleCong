...
function start(){
	...
	if(document.getElementById){
		...
		document.getElementById("title").innerHTML='三角形合同証明問題 問題6　　<button type="button" class="button_sg" value="start" onclick= "start()">始める</button>　<input type="text" name="student_ID" size="30" value="生徒IDが違います。半角で正しく入れましょう。">'
	}
}
//ヒントを押したことをログに残す関数
function hint_katei(){
	...
	if(form_number==23){
		...
		if(boolnum==0){
			if(forcong==3){
				...
				mode="cong";
				congboolean=true;
				makeButton("congCond",150,180,"green",'____________がそれぞれ等しい',"");
...
function ques7_fb2(){
	...
	if(k==3){
		...
		stLine(10+40,70+10,150+60,180+10);
		stLine(100+40,70+10,150+60,180+10);
		stLine(300+40,130+10,150+60,180+10);
	}else if(k<=2){
		diag_history=document.getElementById("diag").innerHTML;
...
function ques7_fb3(){
	if(document.ques7_fb2_select.elements[0].value==2){
		...
		mode="normal";
		document.getElementById("div_congCond").innerHTML='';
		beforeX=80;
		beforeY=180;
...
function ques8_fb2(){
	if(k1==1&&k2==1&fp==1){
		...
		stLine(100+40,70+10,150+60,180+10);
		stLine(300+40,130+10,150+60,180+10);
		beforeX=150;beforeY=180;
		makeButton("congCond",150,180,"green",'____________がそれぞれ等しい',"");
		stLine(150+60,180+10,160+60,260+10);
	}else{
		diag_history=document.getElementById("diag").innerHTML;
